...
trackHit('broken-access-control-easy-1');

$flag = file_exists('/var/flags/flag1.txt') ? trim(file_get_contents('/var/flags/flag1.txt')) : 'IDS{broken_access_control_easy_1_placeholder}';
$users = [
...
if (strpos($uri, '/api/') === 0) {
        if (preg_match('#^/api/users/(\d+)$#', $uri, $m) && $method === 'GET') {
            $token = getAuthToken();
            ...
            if ($requestedId !== intval($payload['user_id'])) {
                trackFlag('broken-access-control-easy-1', $flag);
            }
            jsonResponse([
                'id' => $user['id'],
                ...
                'email' => $user['email'],
                'notes' => $user['notes'],
                'flag' => ($requestedId !== intval($payload['user_id'])) ? $flag : null
            ]);
        }
...
<head>
    ...
    <title>Broken Access Control Easy 1 - IDOR</title>
    <style>
...
<div class="wrap">
    <div class="hero">
        ...
        <h1>Broken Access Control Easy 1 - IDOR</h1>
        <p>User records are fetched directly by ID without ownership checks.</p>
    </div>
    <div class="grid">
        <div class="card">
            <strong>Objective</strong>
            <p>Log in as alice, then request bob's record by changing the user ID.</p>
            <div class="hint"><strong>Hint:</strong> Try GET /api/users/2 after logging in.</div>
