...
trackHit('broken-access-control-easy-4');

$flag = file_exists('/var/flags/flag1.txt') ? trim(file_get_contents('/var/flags/flag1.txt')) : 'IDS{broken_access_control_easy_4_placeholder}';
$users = [
...
if (strpos($uri, '/api/') === 0) {
        if ($uri === '/api/admin/debug/flag' && $method === 'GET') {
            trackFlag('broken-access-control-easy-4', $flag);
            jsonResponse(['debug' => true, 'flag' => $flag, 'env' => 'development']);
        }
...
<head>
    ...
    <title>Broken Access Control Easy 4 - Forceful Browsing</title>
    <style>
...
<div class="wrap">
    <div class="hero">
        ...
        <h1>Broken Access Control Easy 4 - Forceful Browsing</h1>
        <p>A hidden debug endpoint was left exposed without any authentication.</p>
    </div>
    <div class="grid">
        <div class="card">
            <strong>Objective</strong>
            <p>Discover the hidden debug route and request it directly.</p>
            <div class="hint"><strong>Hint:</strong> Try GET /api/admin/debug/flag with no token.</div>
        ...
        <div class="card">
            ...
            <div class="grid" style="grid-template-columns: 120px 1fr; gap: 10px;">
                ...
                <div>
                    <label>URL</label>
                    <input id="url" value="/api/admin/debug/flag">
                </div>
            </div>
