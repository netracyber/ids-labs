...
trackHit('api-security-easy-2');

$flag = file_exists('/var/flags/flag1.txt') ? trim(file_get_contents('/var/flags/flag1.txt')) : 'IDS{api_security_easy_2_placeholder}';
$users = [
...
if (strpos($uri, '/api/') === 0) {
        if ($uri === '/api/profile' && $method === 'GET') {
            ...
            trackFlag('api-security-easy-2', $flag);
            jsonResponse([
                'id' => $user['id'],
                ...
                'debug' => [
                    'api_key' => $flag,
                    'db_dsn' => 'mysql://root:root@localhost/app'
                ]
            ]);
        }
...
<head>
    ...
    <title>API Security Easy 2 - Excessive Data Exposure</title>
    <style>
...
<div class="wrap">
    <div class="hero">
        ...
        <h1>API Security Easy 2 - Excessive Data Exposure</h1>
        <p>The profile endpoint returns far more internal data than it should.</p>
    </div>

    <div class="grid">
        <div class="card">
            <strong>Objective</strong>
            <p>Log in as alice, then inspect your profile response for leaked debug data.</p>
            <div class="hint"><strong>Hint:</strong> Login first, then call GET /api/profile with the token.</div>
        ...
        <div class="card">
            ...
            <div class="grid" style="grid-template-columns: 120px 1fr; gap: 10px;">
                ...
                <div>
                    <label>URL</label>
                    <input id="url" value="/api/profile">
                </div>
            </div>
