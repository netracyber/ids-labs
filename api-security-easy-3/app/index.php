...
trackHit('api-security-easy-3');

$flag = file_exists('/var/flags/flag1.txt') ? trim(file_get_contents('/var/flags/flag1.txt')) : 'IDS{api_security_easy_3_placeholder}';
$users = [
...
if (strpos($uri, '/api/') === 0) {
        if (preg_match('#^/api/users/(\d+)$#', $uri, $m) && $method === 'GET') {
            ...
            if ($requestedId !== intval($payload['user_id'])) {
                trackFlag('api-security-easy-3', $flag);
            }
            jsonResponse([
                'id' => $user['id'],
...
<head>
    ...
    <title>API Security Easy 3 - IDOR</title>
    <style>
...
<div class="wrap">
    <div class="hero">
        ...
        <h1>API Security Easy 3 - IDOR</h1>
        <p>User data is returned by ID without checking ownership.</p>
    </div>

    <div class="grid">
        <div class="card">
            <strong>Objective</strong>
            <p>Log in as alice and request bob's profile by changing the user ID in the path.</p>
