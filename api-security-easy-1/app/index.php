...
trackHit('api-security-easy-1');

$flag = file_exists('/var/flags/flag1.txt') ? trim(file_get_contents('/var/flags/flag1.txt')) : 'IDS{api_security_easy_1_placeholder}';
$users = [
...
if (strpos($uri, '/api/') === 0) {
        if ($uri === '/api/admin/users' && $method === 'GET') {
            trackFlag('api-security-easy-1', $flag);
            jsonResponse(['users' => array_values($users), 'secret' => $flag]);
        }
...
<head>
    ...
    <title>API Security Easy 1 - Broken Authentication</title>
    <style>
...
<div class="wrap">
    <div class="hero">
        ...
        <h1>API Security Easy 1 - Broken Authentication</h1>
        <p>A public admin endpoint leaks all user records when requested directly.</p>
    </div>

    <div class="grid">
        <div class="card">
            <strong>Objective</strong>
            <p>Fetch the admin users list without logging in.</p>
            <div class="hint"><strong>Hint:</strong> Send GET /api/admin/users without any token.</div>
        ...
        <div class="card">
            ...
            <div class="grid" style="grid-template-columns: 120px 1fr; gap: 10px;">
                ...
                <div>
                    <label>URL</label>
                    <input id="url" value="/api/admin/users">
                </div>
            </div>
