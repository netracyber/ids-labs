...
# Script to restart the additional XSS labs with updated content

echo "Stopping existing servers..."
pkill -f "http.server 8007" 2>/dev/null
pkill -f "http.server 8008" 2>/dev/null
pkill -f "http.server 8009" 2>/dev/null
...
# Copy each updated lab file to its respective directory
...
echo "Updated XSS labs restarted:"
