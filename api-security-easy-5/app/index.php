...
trackHit('api-security-easy-5');

$flag = file_exists('/var/flags/flag1.txt') ? trim(file_get_contents('/var/flags/flag1.txt')) : 'IDS{api_security_easy_5_placeholder}';
$users = [
...
if (strpos($uri, '/api/') === 0) {
        if ($uri === '/api/admin/flag' && $method === 'GET') {
            ...
            if (($payload['role'] ?? '') === 'admin') {
                trackFlag('api-security-easy-5', $flag);
                jsonResponse(['message' => 'Welcome admin', 'flag' => $flag]);
            }
...
<head>
    ...
    <title>API Security Easy 5 - JWT Token Tampering</title>
    <style>
...
<div class="wrap">
    <div class="hero">
        <div class="tag">API Security • Easy Lab</div>
        <h1>API Security Easy 5 - JWT Token Tampering</h1>
        <p>The API trusts the token payload without verifying the signature.</p>
    </div>

    <div class="grid">
        <div class="card">
            <strong>Objective</strong>
            <p>Log in as alice, edit the JWT payload so your role becomes admin, then access the admin flag.</p>
            <div class="hint"><strong>Hint:</strong> Change the token payload to role=admin and reuse it on GET /api/admin/flag.</div>
