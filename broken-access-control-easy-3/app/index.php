...
trackHit('broken-access-control-easy-3');

$flag = file_exists('/var/flags/flag1.txt') ? trim(file_get_contents('/var/flags/flag1.txt')) : 'IDS{broken_access_control_easy_3_placeholder}';
$users = [
...
if (strpos($uri, '/api/') === 0) {
        if ($uri === '/api/admin/flag' && $method === 'POST') {
            ...
            if (isset($payload['role'])) {
                trackFlag('broken-access-control-easy-3', $flag);
                jsonResponse(['message' => 'Admin flag retrieved', 'flag' => $flag]);
            }
...
<head>
    ...
    <title>Broken Access Control Easy 3 - Vertical Privilege Escalation</title>
    <style>
...
<div class="wrap">
    <div class="hero">
        ...
        <h1>Broken Access Control Easy 3 - Vertical Privilege Escalation</h1>
        <p>The admin endpoint only checks that a role field exists, not that it equals admin.</p>
    </div>
    <div class="grid">
        <div class="card">
            <strong>Objective</strong>
            <p>Log in as alice and call the admin flag endpoint with the regular user token.</p>
            <div class="hint"><strong>Hint:</strong> Try POST /api/admin/flag after logging in as a normal user.</div>
        ...
        <div class="card">
            ...
            <div class="grid" style="grid-template-columns: 120px 1fr; gap: 10px;">
                ...
                <div>
                    <label>URL</label>
                    <input id="url" value="/api/admin/flag">
                </div>
            </div>
