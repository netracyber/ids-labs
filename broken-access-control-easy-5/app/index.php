...
trackHit('broken-access-control-easy-5');

$flag = file_exists('/var/flags/flag1.txt') ? trim(file_get_contents('/var/flags/flag1.txt')) : 'IDS{broken_access_control_easy_5_placeholder}';
$users = [
...
if (strpos($uri, '/api/') === 0) {
        if ($uri === '/api/admin/flag.json' && $method === 'GET') {
            trackFlag('broken-access-control-easy-5', $flag);
            jsonResponse(['flag' => $flag, 'format' => 'json']);
        }
...
<head>
    ...
    <title>Broken Access Control Easy 5 - Path Bypass</title>
    <style>
...
<div class="wrap">
    <div class="hero">
        ...
        <h1>Broken Access Control Easy 5 - Path Bypass</h1>
        <p>A hidden file-suffixed admin path bypasses the intended access control rule.</p>
    </div>
    <div class="grid">
        <div class="card">
            <strong>Objective</strong>
            <p>Try the alternate admin path that ends in .json.</p>
            <div class="hint"><strong>Hint:</strong> Request GET /api/admin/flag.json directly.</div>
        ...
        <div class="card">
            ...
            <div class="grid" style="grid-template-columns: 120px 1fr; gap: 10px;">
                ...
                <div>
                    <label>URL</label>
                    <input id="url" value="/api/admin/flag.json">
                </div>
            </div>
